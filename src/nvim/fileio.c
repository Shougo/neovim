...
{
  int result = 0;
  char_u *exp = vim_strsave((char_u *)name);
  Directory dir;

  if (os_isrealdir(name) && os_scandir(&dir, (const char *)exp)) {
    garray_T ga;

    readdir_core(&ga, &dir, (const char *)exp, NULL, false);
